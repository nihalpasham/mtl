...
pub fn dotprod<U: Mul<Output = U> + Copy>(a: &Vec<U>, b: &Vec<U>, kernel: Kernels) -> Vec<U> {
    ...
    match a.len() == b.len() {
        true => {
            ...
            gpu_buffer_res =
                device.new_buffer(a.len() as u64, MTLResourceOptions::StorageModeShared);
        }
        false => {
    ...
    let function = match kernel {
        Kernels::F16(s) => lib.get_function(s, None).unwrap(),
...
pub fn run_gpu<U: Mul<Output = U> + Copy>(a: &Vec<U>, b: &Vec<U>, kernel: &str) -> Vec<U> {
    let kernel_hostname = match kernel {
        ...
        "dotprod_float" => Kernels::F16(kernel),
        _ => unimplemented!(),
    };
